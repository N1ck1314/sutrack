...
def generate_html_document(trackers: List[Tracker], sequences: List[Sequence], reports, storage: Storage, metadata: dict = None):
    ...
    for tracker in trackers:
        try:
            # 尝试从 results/{tracker_id}/fps_summary.txt 读取FPS数据
            tracker_id = tracker.reference
            fps_file_path = os.path.join(storage._directory, "..", "..", "results", tracker_id, "fps_summary.txt")
            fps_file_path = os.path.normpath(fps_file_path)
            ...
            if os.path.exists(fps_file_path):
                with open(fps_file_path, 'r') as f:
                    ...
                    if fps_info:
                        avg_fps = fps_info.get('Average FPS', 'N/A')
                        fps_data[tracker.label] = avg_fps
        except Exception:
            pass
    ...
    with doc:
        ...
        with div(id="wrapper"):
            ...
            if fps_data:
                h2("FPS Performance")
                with table(cls="pure-table pure-table-horizontal"):
                    with thead():
                        with tr():
                            th("Tracker")
                            th("Average FPS")
                    with tbody():
                        for tracker_label, avg_fps in fps_data.items():
                            with tr():
                                td(tracker_label)
                                td(avg_fps)
