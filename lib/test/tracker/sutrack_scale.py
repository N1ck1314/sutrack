...
class SUTRACK_SCALE(BaseTracker):
    def __init__(self, params, dataset_name):
        super(SUTRACK_SCALE, self).__init__(params)
        network = build_sutrack_scale(params.cfg)
    ...
    def extract_token_from_nlp_clip(self, nlp_in):
        if nlp_in is not None:
            text_seq = clip.tokenize([nlp_in])
        else:
            text_seq = clip.tokenize(["default"])
        return text_seq, len(text_seq)


def get_tracker_class():
    return SUTRACK_SCALE
