...
cfg.MODEL.ENCODER.TYPE = "fastitpnt" # encoder model - using tiny for efficiency
cfg.MODEL.ENCODER.DROP_PATH = 0
cfg.MODEL.ENCODER.PRETRAIN_TYPE = "pretrained/itpn/fast_itpn_tiny_224.pth" #
...
# MODEL.DSCL - Decoupled Spatio-Temporal Consistency Learning
cfg.MODEL.USE_DSCL = True  # 启用DSCL模块
cfg.MODEL.DSCL = edict()
cfg.MODEL.DSCL.NUM_FRAMES = 2  # 时间帧数
cfg.MODEL.DSCL.SPATIAL_HEADS = 8  # 空间注意力头数
cfg.MODEL.DSCL.TEMPORAL_HEADS = 4  # 时间注意力头数
cfg.MODEL.DSCL.DROP_PATH = 0.1  # Drop path rate

# MODEL.SS_LOSS - Self-Supervised Loss
cfg.MODEL.USE_SS_LOSS = True  # 启用自监督损失
cfg.MODEL.SS_LOSS = edict()
cfg.MODEL.SS_LOSS.TEMPERATURE = 0.5  # 对比学习温度系数
cfg.MODEL.SS_LOSS.CONTRASTIVE_WEIGHT = 1.0  # 对比损失权重
cfg.MODEL.SS_LOSS.TEMPORAL_WEIGHT = 0.5  # 时间一致性损失权重
cfg.MODEL.SS_LOSS.USE_COSINE = True  # 使用余弦相似度
