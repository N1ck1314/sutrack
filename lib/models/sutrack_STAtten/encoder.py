"""  
Encoder modules: we use ITPN for the encoder with STAtten support.
"""

from torch import nn
from lib.utils.misc import is_main_process
from lib.models.sutrack_STAtten import fastitpn as fastitpn_module
from lib.models.sutrack_STAtten import itpn as oriitpn_module
...
class Encoder(EncoderBase):
    """ViT encoder with STAtten support."""
    def __init__(self, name: str,
        ...
        if "fastitpn" in name.lower():
            encoder = getattr(fastitpn_module, name)(
                ...
                # STAtten相关参数
                use_statten=cfg.MODEL.ENCODER.get('USE_STATTEN', True),
                statten_mode=cfg.MODEL.ENCODER.get('STATTEN_MODE', 'STAtten'),
                use_snn=cfg.MODEL.ENCODER.get('USE_SNN', True),
            )
            if "itpnb" in name:
                num_channels = 512
            elif "itpnl" in name:
