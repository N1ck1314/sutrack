...
# Import STAtten module
from .statten import STAttenAttention
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 swiglu=False,
                 naiveswiglu=False,
                 use_statten=False,  # 新增：是否使用STAtten
                 statten_mode="STAtten",  # STAtten模式
        ...
        # 根据use_statten选择注意力模块
        if use_statten:
            self.attn = STAttenAttention(
                dim, num_heads=num_heads, qkv_bias=qkv_bias,
                attn_drop=attn_drop, proj_drop=drop,
                attention_mode=statten_mode,
                use_snn=use_snn
...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
                 ...
                 use_statten=True,  # 新增：是否使用STAtten（默认启用）
                 statten_mode="STAtten",  # STAtten模式
                 use_snn=True,  # 是否使用脉冲神经网络（默认启用）
                 **kwargs):
        super().__init__()
        self.search_size = search_size
        self.template_size = template_size
        ...
        # STAtten相关参数
        self.use_statten = use_statten
        self.statten_mode = statten_mode
        ...
        self.build_blocks(
            ...
            convmlp=convmlp,
            use_statten=use_statten,  # 传递STAtten参数
            statten_mode=statten_mode,
            use_snn=use_snn,
        )
