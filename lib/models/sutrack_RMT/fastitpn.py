...
# Import CMSA module
from .cmsa import MultiModalFusionWithCMSA
...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
                 ...
                 use_cmsa=True,  # 新增：是否使用CMSA（默认启用）
                 cmsa_mode='cmsa',  # CMSA模式
                 use_ssm=True,  # 是否使用SSM
                 use_rmt=True,  # 新增：是否使用RMT（默认禁用）
                 rmt_layers=[],  # 新增：使用RMT的层索引
                 rmt_num_heads=8,  # 新增：RMT注意力头数
                 **kwargs):
        super().__init__()
        self.search_size = search_size
        self.template_size = template_size
        ...
        # CMSA相关参数
        self.use_cmsa = use_cmsa
        self.cmsa_mode = cmsa_mode
        self.use_ssm = use_ssm
        
        # RMT相关参数
        self.use_rmt = use_rmt
        self.rmt_layers = rmt_layers
        self.rmt_num_heads = rmt_num_heads
        
        # 初始化RMT模块（如果启用）
        if self.use_rmt and len(self.rmt_layers) > 0:
            from .rmt_module import RetNetRelPos2d
            self.rmt_rel_pos_encoder = RetNetRelPos2d(
                embed_dim=embed_dim,
                num_heads=rmt_num_heads,
                initial_value=1,
                heads_range=3
            )
        else:
            self.rmt_rel_pos_encoder = None
        ...
        if self.use_cmsa:
            # 计算patch数量和尺寸
            h_search = w_search = search_size // patch_size
            h_template = w_template = template_size // patch_size
            
            # 为search和template创建CMSA模块
            self.cmsa_search = MultiModalFusionWithCMSA(
                dim=embed_dim,
                h=h_search,
                w=w_search,
                use_ssm=use_ssm,
                fusion_mode=cmsa_mode
            )
            self.cmsa_template = MultiModalFusionWithCMSA(
                dim=embed_dim,
                h=h_template,
                w=w_template,
                use_ssm=use_ssm,
            ...
            self.cmsa_search = None
            self.cmsa_template = None
    ...
    def prepare_tokens_with_masks(self, template_list, search_list, template_anno_list, text_src, task_index):
        ...
        # 应用CMSA进行多模态融合（如果in_chans=6，说明是双模态）
        if self.use_cmsa and z.size(1) > self.embed_dim // 4:  # 检查是否是多模态
            # z和x都可能是双模态，需要拆分后融合
            # 假设是convmlp=True，这里是[bn, c, h, w]格式
            if not self.convmlp:
                # 如果不是convmlp，需要先转换
                pass
            # 在后续的token处理中应用CMSA
        ...
        if self.use_cmsa:
            # 检查是否是多模态数据（通过检查通道数）
            # 如果是in_chans=6，说明是双模态（RGB+Depth/Thermal）
            # 此时x和z的通道维度已经是融合后的embed_dim
            # 我们需要在之前处理，或者假设模态信息已经嵌入到token中
            # 这里我们保留接口，在后续使用
            pass
    ...
    def forward_features(self, template_list, search_list,template_anno_list, text_src, task_index):
        ...
        # 计算特征图尺寸用于RMT
        total_tokens = xz.size(1)
        # 假设正方形特征图，计算边长
        feature_side = int(math.sqrt(total_tokens))
        if feature_side * feature_side != total_tokens:
            # 如果不是正方形，使用近似值
            feature_side = int(math.sqrt(total_tokens - 1)) if self.cls_token is not None else int(math.sqrt(total_tokens))
        
        # 生成RMT相对位置编码（如果启用）
        if self.use_rmt and self.rmt_rel_pos_encoder is not None and len(self.rmt_layers) > 0:
            rmt_rel_pos = self.rmt_rel_pos_encoder((feature_side, feature_side), chunkwise_recurrent=True)
        else:
            rmt_rel_pos = None
        ...
        for i, blk in enumerate(self.blocks[-self.num_main_blocks:]):
            # 检查是否在RMT层中
            if self.use_rmt and (len(self.rmt_layers) > 0) and (i in self.rmt_layers):
                # 使用RMT处理（这里需要特殊处理，因为blk是标准的Transformer块）
                # 为了简化，我们仍然使用标准块，但在未来可以替换为RMT块
