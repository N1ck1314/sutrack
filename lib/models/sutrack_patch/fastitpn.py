...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        # === 新增：多尺度Token融合模块（针对无人机尺度变化） ===
        self.use_multiscale_fusion = kwargs.get('use_multiscale_fusion', True)
        self.multiscale_fusion_weight = kwargs.get('multiscale_fusion_weight', 0.5)
        
        if self.use_multiscale_fusion:
            # 小尺度 patch embedding (patch_size / 2，捕获细节)
            self.patch_embed_small = ConvPatchEmbed(
                ...
                stop_grad_conv1=stop_grad_conv1,
                norm_layer=norm_layer if patch_norm else None
            ) if convmlp else PatchEmbed(
                img_size=search_size // 2,
                patch_size=patch_size // 2,
                in_chans=in_chans,
                embed_dim=mlvl_dims['4'],
                norm_layer=norm_layer if patch_norm else None
            ...
            # 大尺度 patch embedding (patch_size * 2，捕获全局)
            self.patch_embed_large = ConvPatchEmbed(
                search_size=search_size * 2,
                template_size=template_size * 2,
                patch_size=patch_size * 2,
                in_chans=in_chans,
            ...
            # 尺度融合权重学习网络
            self.scale_fusion_net = nn.Sequential(
                nn.Linear(mlvl_dims['4'] * 3, mlvl_dims['4']),
                nn.LayerNorm(mlvl_dims['4']),
                nn.ReLU(),
                nn.Linear(mlvl_dims['4'], 3),
                nn.Softmax(dim=-1)
            )
        # ============================================================
    ...
    def prepare_tokens_with_masks(self, template_list, search_list, template_anno_list, text_src, task_index):
        ...
        # === 新增：多尺度Token融合（适应无人机尺度变化） ===
        if self.use_multiscale_fusion:
            # 保存原始图像用于多尺度处理
            z_orig = torch.stack(template_list, dim=1).view(-1, *template_list[0].size()[1:])
            x_orig = torch.stack(search_list, dim=1).view(-1, *search_list[0].size()[1:])
            
            # 小尺度特征（捕获细节，适合目标较大时）
            z_small_input = F.interpolate(z_orig, size=(z_orig.size(-2)//2, z_orig.size(-1)//2), mode='bilinear', align_corners=False)
            x_small_input = F.interpolate(x_orig, size=(x_orig.size(-2)//2, x_orig.size(-1)//2), mode='bilinear', align_corners=False)
            z_small = self.patch_embed_small(z_small_input)
            x_small = self.patch_embed_small(x_small_input)
            
            # 大尺度特征（捕获全局上下文，适合目标较小时）
            z_large_input = F.interpolate(z_orig, size=(z_orig.size(-2)*2, z_orig.size(-1)*2), mode='bilinear', align_corners=False)
            x_large_input = F.interpolate(x_orig, size=(x_orig.size(-2)*2, x_orig.size(-1)*2), mode='bilinear', align_corners=False)
            z_large = self.patch_embed_large(z_large_input)
            x_large = self.patch_embed_large(x_large_input)
            
            # 将不同尺度的特征插值到相同大小
            # z, x 的形状: (bn, c, h, w)
            target_size = (z.size(-2), z.size(-1))
            z_small_up = F.interpolate(z_small, size=target_size, mode='bilinear', align_corners=False)
            z_large_up = F.interpolate(z_large, size=target_size, mode='bilinear', align_corners=False)
            x_small_up = F.interpolate(x_small, size=target_size, mode='bilinear', align_corners=False)
            x_large_up = F.interpolate(x_large, size=target_size, mode='bilinear', align_corners=False)
            
            # 将特征展平为 tokens
            z_orig_flat = z.flatten(2).transpose(1, 2)  # (bn, L, c)
            z_small_flat = z_small_up.flatten(2).transpose(1, 2)
            z_large_flat = z_large_up.flatten(2).transpose(1, 2)
            x_orig_flat = x.flatten(2).transpose(1, 2)
            x_small_flat = x_small_up.flatten(2).transpose(1, 2)
            x_large_flat = x_large_up.flatten(2).transpose(1, 2)
            
            # 拼接三个尽度的特征用于权重计算
            z_concat = torch.cat([z_orig_flat, z_small_flat, z_large_flat], dim=-1)  # (bn, L, 3c)
            x_concat = torch.cat([x_orig_flat, x_small_flat, x_large_flat], dim=-1)
            
            # 学习自适应的尺度融合权重
            z_weights = self.scale_fusion_net(z_concat.mean(dim=1))  # (bn, 3) 全局平均
            x_weights = self.scale_fusion_net(x_concat.mean(dim=1))  # (bn, 3)
            
            # 应用加权融合
            z_fused_flat = (z_orig_flat * z_weights[:, 0:1].unsqueeze(1) + 
                           z_small_flat * z_weights[:, 1:2].unsqueeze(1) + 
                           z_large_flat * z_weights[:, 2:3].unsqueeze(1))
            x_fused_flat = (x_orig_flat * x_weights[:, 0:1].unsqueeze(1) + 
                           x_small_flat * x_weights[:, 1:2].unsqueeze(1) + 
                           x_large_flat * x_weights[:, 2:3].unsqueeze(1))
            
            # 转换回空间形状
            z = z_fused_flat.transpose(1, 2).view_as(z)  # (bn, c, h, w)
            x = x_fused_flat.transpose(1, 2).view_as(x)
        # ================================================================
