...
class Fast_iTPN(nn.Module):
    ...
    def build_blocks(self,
        ...
        # ========== DEBUG: 打印模型结构信息 ==========
        print("\n" + "="*60)
        print("[DEBUG] Fast-iTPN 模型结构信息：")
        print("="*60)
        print(f"Stage1层数: {depths[0]}")
        print(f"Stage2层数: {depths[1]}")
        print(f"Stage3主Transformer层数: {depths[2]}")
        print(f"总层数: {depths[0] + depths[1] + depths[2] + 2} (包含2个PatchMerge层)")
        print(f"\n各阶段维度信息:")
        print(f"  Stage1 (1/4分辨率): dim={dims['4']}")
        print(f"  Stage2 (1/8分辨率): dim={dims['8']}")
        print(f"  Stage3 (1/16分辨率): dim={dims['16']}, num_heads={num_heads}")
        print(f"\nMLP比例: bridge_mlp_ratio={bridge_mlp_ratio}, mlp_ratio={mlp_ratio}")
        print(f"是否使用ConvMLP: {convmlp}")
        print("="*60 + "\n")
        # ============================================
        ...
        if convmlp:
            print(f"[DEBUG] 构建Stage1: {depths[0]}个ConvMlpBlock (dim={dims['4']})")
            self.blocks.extend([
                ConvMlpBlock(
                    dim=dims['4'],
            ...
            print(f"[DEBUG] 添加PatchMerge: {dims['4']} -> {dims['8']}")
            self.blocks.append(ConvPatchMerge(dims['4'], norm_layer))
            print(f"[DEBUG] 构建Stage2: {depths[1]}个ConvMlpBlock (dim={dims['8']})")
            self.blocks.extend([
                ConvMlpBlock(
                    dim=dims['8'],
                    mlp_ratio=bridge_mlp_ratio,
            ...
            print(f"[DEBUG] 添加PatchMerge: {dims['8']} -> {dims['16']}")
        ...
        ######### stage 3 ########
        print(f"[DEBUG] 构建Stage3: {depths[2]}个Transformer Block (dim={dims['16']}, heads={num_heads})")
        self.blocks.extend([
            Block(
                dim=dims['16'],
                num_heads=num_heads,
        ...
        print(f"[DEBUG] 模型构建完成！总共 {len(self.blocks)} 个blocks")
        print("="*60 + "\n")
