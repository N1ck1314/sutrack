...
def build_sutrack_scale(cfg):
    """
    Build SUTrack Scale (Multi-scale Fusion + Depth-conditioned Token Version)
    结合方向2：显式尺度选择（利用Fast-iTPN的金字塔特征）
    结合方向3：深度条件Token & 3D位置编码（适配RGBD跟踪）
    """
