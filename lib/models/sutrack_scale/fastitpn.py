...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        if self.use_scale_selector:
            # 对每个尺度的特征池化并计算权重
            self.scale_selector = nn.ModuleDict({
                'pool_s': nn.AdaptiveAvgPool2d(1),  # 小尺度池化
                'pool_m': nn.AdaptiveAvgPool2d(1),  # 中尺度池化
                'pool_l': nn.AdaptiveAvgPool2d(1),  # 大尺度池化
                'weight_net': nn.Sequential(
                    nn.Linear(mlvl_dims['4'] + mlvl_dims['8'] + mlvl_dims['16'], embed_dim // 4),
            ...
            # 尺度融合投影层（将不同维度统一到 embed_dim）
            self.scale_proj_s = nn.Linear(mlvl_dims['4'], embed_dim)
            self.scale_proj_m = nn.Linear(mlvl_dims['8'], embed_dim)
        ...
        if self.use_depth_conditioning:
            # 3D位置编码生成网络
            self.depth_pos_embed_net = nn.Sequential(
                nn.Linear(3, embed_dim // 2),  # (X, Y, Z) -> hidden
                nn.LayerNorm(embed_dim // 2),
                nn.ReLU(),
                nn.Linear(embed_dim // 2, embed_dim)
            )
            # 深度条件的注意力Bias参数
            self.depth_attn_bias_scale = nn.Parameter(torch.ones(1) * 0.1)
        # ================================================================
    ...
    def prepare_tokens_with_masks(self, template_list, search_list, template_anno_list, text_src, task_index):
        ...
        # === 方向2：从 Fast-iTPN 金字塔中提取多尺度特征 ===
        if self.use_scale_selector:
            # 保存中间层特征用于尺度选择
            z_multiscale = []
            x_multiscale = []
            
            # 处理 stage1&2，收集三个尺度的特征
            z_curr, x_curr = z, x
            for i, blk in enumerate(self.blocks[:-self.num_main_blocks]):
                z_curr = checkpoint.checkpoint(blk, z_curr) if self.grad_ckpt else blk(z_curr)
                x_curr = checkpoint.checkpoint(blk, x_curr) if self.grad_ckpt else blk(x_curr)
                
                # 在 PatchMerge 之前保存特征（小、中、大三个尺度）
                if hasattr(blk, '__class__') and ('PatchMerge' in blk.__class__.__name__ or 
                                                   'ConvPatchMerge' in blk.__class__.__name__):
                    # PatchMerge之前的是下采样的边界
                    pass
                elif i == 0 or (i > 0 and ('Merge' in self.blocks[i-1].__class__.__name__)):
                    # 每个stage的第一个block之后，保存特征
                    z_multiscale.append(z_curr)
                    x_multiscale.append(x_curr)
            
            # 确保我们有至少两个尺度（如果不够，使用最后一层）
            if len(z_multiscale) < 3:
                z_multiscale.append(z_curr)
                x_multiscale.append(x_curr)
            
            # 取前三个尺度：小(s)、中(m)、大(l)
            z_s, z_m, z_l = z_multiscale[0], z_multiscale[min(1, len(z_multiscale)-1)], z_curr
            x_s, x_m, x_l = x_multiscale[0], x_multiscale[min(1, len(x_multiscale)-1)], x_curr
            
            # 池化并计算尺度权重
            z_s_pool = self.scale_selector['pool_s'](z_s).flatten(1)  # (bn, c_s)
            z_m_pool = self.scale_selector['pool_m'](z_m).flatten(1)  # (bn, c_m)
            z_l_pool = self.scale_selector['pool_l'](z_l).flatten(1)  # (bn, c_l)
            z_concat = torch.cat([z_s_pool, z_m_pool, z_l_pool], dim=1)
            z_scale_weights = self.scale_selector['weight_net'](z_concat)  # (bn, 3)
            
            x_s_pool = self.scale_selector['pool_s'](x_s).flatten(1)
            x_m_pool = self.scale_selector['pool_m'](x_m).flatten(1)
            x_l_pool = self.scale_selector['pool_l'](x_l).flatten(1)
            x_concat = torch.cat([x_s_pool, x_m_pool, x_l_pool], dim=1)
            x_scale_weights = self.scale_selector['weight_net'](x_concat)  # (bn, 3)
            
            # 将不同尺度投影到相同维度
            z_s_flat = self.scale_proj_s(z_s.flatten(2).transpose(1, 2))  # (bn, L_s, embed_dim)
            z_m_flat = self.scale_proj_m(z_m.flatten(2).transpose(1, 2))  # (bn, L_m, embed_dim)
            z_l_flat = self.scale_proj_l(z_l.flatten(2).transpose(1, 2))  # (bn, L_l, embed_dim)
            
            x_s_flat = self.scale_proj_s(x_s.flatten(2).transpose(1, 2))
            x_m_flat = self.scale_proj_m(x_m.flatten(2).transpose(1, 2))
            x_l_flat = self.scale_proj_l(x_l.flatten(2).transpose(1, 2))
            
            # 插值到目标尺寸（使用最大尺度作为目标）
            target_len = z_l_flat.size(1)
            if z_s_flat.size(1) != target_len:
                z_s_flat = F.interpolate(z_s_flat.transpose(1, 2), size=target_len, mode='linear', align_corners=False).transpose(1, 2)
            if z_m_flat.size(1) != target_len:
                z_m_flat = F.interpolate(z_m_flat.transpose(1, 2), size=target_len, mode='linear', align_corners=False).transpose(1, 2)
            
            if x_s_flat.size(1) != target_len:
                x_s_flat = F.interpolate(x_s_flat.transpose(1, 2), size=target_len, mode='linear', align_corners=False).transpose(1, 2)
            if x_m_flat.size(1) != target_len:
                x_m_flat = F.interpolate(x_m_flat.transpose(1, 2), size=target_len, mode='linear', align_corners=False).transpose(1, 2)
            
            # 加权融合
            z = (z_s_flat * z_scale_weights[:, 0:1].unsqueeze(1) +
                 z_m_flat * z_scale_weights[:, 1:2].unsqueeze(1) +
                 z_l_flat * z_scale_weights[:, 2:3].unsqueeze(1))  # (bn, L, embed_dim)
            
            x = (x_s_flat * x_scale_weights[:, 0:1].unsqueeze(1) +
                 x_m_flat * x_scale_weights[:, 1:2].unsqueeze(1) +
                 x_l_flat * x_scale_weights[:, 2:3].unsqueeze(1))
        else:
            # 不使用尺度选择时，正常处理
        ...
        if self.use_depth_conditioning:
            ...
            import math
            bn = x.size(0)
            num_patches_x = x.size(1)
            num_patches_z = z.size(1)
            
            # 生成归一化的 (x, y) 坐标
            h_x = int(math.sqrt(num_patches_x))
            w_x = h_x
            y_coords = torch.arange(h_x, device=x.device).float() / h_x
            x_coords = torch.arange(w_x, device=x.device).float() / w_x
            yy, xx = torch.meshgrid(y_coords, x_coords, indexing='ij')
            
            # 模拟深度（使用中心距离作为伪深度）
            center_y, center_x = 0.5, 0.5
            depth_sim = torch.sqrt((xx - center_x)**2 + (yy - center_y)**2)  # (h, w)
            depth_sim = 1.0 - depth_sim  # 反转：中心深度大
            
            # 构建 3D 坐标 (X, Y, Z)
            xyz_coords = torch.stack([xx, yy, depth_sim], dim=-1)  # (h, w, 3)
            xyz_coords = xyz_coords.view(-1, 3)  # (L, 3)
            xyz_coords = xyz_coords.unsqueeze(0).expand(bn, -1, -1)  # (bn, L, 3)
            
            # 通过 MLP 生成 3D 位置编码
            pos_3d_x = self.depth_pos_embed_net(xyz_coords)  # (bn, L, embed_dim)
            
            # 对 template 生成类似的 3D 位置编码
            h_z = int(math.sqrt(num_patches_z))
            w_z = h_z
            y_coords_z = torch.arange(h_z, device=z.device).float() / h_z
            x_coords_z = torch.arange(w_z, device=z.device).float() / w_z
            yy_z, xx_z = torch.meshgrid(y_coords_z, x_coords_z, indexing='ij')
            depth_sim_z = torch.sqrt((xx_z - center_x)**2 + (yy_z - center_y)**2)
            depth_sim_z = 1.0 - depth_sim_z
            xyz_coords_z = torch.stack([xx_z, yy_z, depth_sim_z], dim=-1).view(-1, 3)
            xyz_coords_z = xyz_coords_z.unsqueeze(0).expand(bn, -1, -1)
            pos_3d_z = self.depth_pos_embed_net(xyz_coords_z)  # (bn, L_z, embed_dim)
