...
# Import CMA module
from .cma import MultiModalFusionWithCMA
...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
                 ...
                 use_cma=True,  # 新增：是否使用CMA（默认启用）
                 cma_mode='cma',  # CMA融合模式
                 **kwargs):
        super().__init__()
        self.search_size = search_size
        self.template_size = template_size
        ...
        # 初始化CMA模块（用于多模态融合）
        if self.use_cma:
            # 为search和template创建CMA融合模块
            self.cma_fusion = MultiModalFusionWithCMA(
                dim=embed_dim,
                fusion_mode=cma_mode
            )
        else:
            self.cma_fusion = None
