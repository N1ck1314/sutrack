...
class BlockWithSCSA(nn.Module):
    """
    Transformer Block with integrated SCSA (Spatial-Channel Synergistic Attention)
    
    集成SCSA的Transformer Block:
    - 在注意力机制之后应用SCSA模块
    - SCSA对特征图进行空间-通道协同增强
    """

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
                 attn_drop=0., drop_path=0., init_values=None, norm_layer=nn.LayerNorm,
        ...
        # Add SCSA module
        self.use_scsa = use_scsa and with_attn
        if self.use_scsa:
            self.scsa = SCSA(
                dim=dim,
            ...
            # For converting between (B, N, C) and (B, C, H, W) formats
            self.hw_size = None
        ...
        if init_values is not None and init_values > 0:
            ...
            self.gamma_2 = nn.Parameter(init_values * torch.ones((dim)), requires_grad=True)
            if self.use_scsa:
                self.gamma_scsa = nn.Parameter(init_values * torch.ones((dim)), requires_grad=True)
        else:
            self.gamma_1, self.gamma_2 = None, None
            if self.use_scsa:
                self.gamma_scsa = None
    ...
    def forward(self, x, rel_pos_bias=None, attn_mask=None):
        # SCSA 不适用于 Transformer 的序列格式 (B, N, C)
        # 因为包含 template + search + cls/text tokens，无法简单重塑为空间格式
        # 保持标准的 Transformer Block 行为
...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        self.scsa_reduction_ratio = scsa_reduction_ratio
        ...
        self.build_blocks(
            ...
            convmlp=convmlp,
            use_scsa=self.use_scsa,
            scsa_reduction_ratio=self.scsa_reduction_ratio,
        )
    ...
    def build_blocks(self,
        ...
        ######### stage 3 ########
        # Use BlockWithSCSA for stage 3 to add SCSA attention mechanism
        self.blocks.extend([
            BlockWithSCSA(
                dim=dims['16'],
                num_heads=num_heads,
