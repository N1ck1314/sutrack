...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        self.selective_depth_threshold = selective_depth_threshold
        self.layer_depth_probs = []  # 存储各层的深度使用概率
        ...
        # Initialize Selective Depth Integration module
        if self.use_selective_depth:
            from lib.models.sutrack_select.selective_depth_modules import SelectiveDepthIntegration
            self.selective_depth_module = SelectiveDepthIntegration(
                dim=embed_dim,
                num_layers=depth,
                threshold=selective_depth_threshold
            )
        else:
            self.selective_depth_module = None
    ...
    def forward_features(self, template_list, search_list,template_anno_list, text_src, task_index):
        ...
        # Clear previous depth probs if using selective depth
        if self.use_selective_depth:
            self.layer_depth_probs = []

        rel_pos_bias = self.rel_pos_bias() if self.rel_pos_bias is not None else None
        
        # Process through transformer blocks
        for layer_idx, blk in enumerate(self.blocks[-self.num_main_blocks:]):
            if self.use_selective_depth and self.selective_depth_module is not None:
                # For selective depth: assume depth features are None for now
                # In actual usage, depth features would be extracted separately
                depth_feat = None  # TODO: Extract depth features if available
                xz, layer_prob = self.selective_depth_module(xz, depth_feat, layer_idx)
                self.layer_depth_probs.append(layer_prob)
...
@register_model
def fastitpnt(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", 
              use_selective_depth=False, selective_depth_threshold=0.5, **kwargs):
    model = Fast_iTPN(
        ...
        subln=True,
        pos_type=pos_type,
        use_selective_depth=use_selective_depth,
        selective_depth_threshold=selective_depth_threshold,
        **kwargs)
    model.default_cfg = _cfg()
    if pretrained and pretrain_type:  # Only load if pretrain_type is not empty
...
@register_model
def fastitpnb(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", 
              use_selective_depth=False, selective_depth_threshold=0.5, **kwargs):
    model = Fast_iTPN(
