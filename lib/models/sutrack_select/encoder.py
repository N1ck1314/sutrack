"""
Encoder modules: we use ITPN for the encoder with Selective Depth Integration.
"""
...
class EncoderBase(nn.Module):
    ...
    def __init__(self, encoder: nn.Module, train_encoder: bool, open_layers: list, num_channels: int):
        ...
        # Selective Depth Integration support
        if getattr(self.body, 'use_selective_depth', False):
            from lib.models.sutrack_select.selective_depth_modules import DepthSelectionLoss
            self.selection_loss_fn = DepthSelectionLoss()
    ...
    def get_selection_loss(self):
        """获取深度选择损失"""
        if hasattr(self, 'selection_loss_fn') and len(self.body.layer_depth_probs) > 0:
            return self.selection_loss_fn(self.body.layer_depth_probs)
        return 0.0
    ...
    def __init__(self, name: str,
        ...
        if "fastitpn" in name.lower():
            encoder = getattr(fastitpn_module, name)(
                ...
                selective_depth_threshold=cfg.MODEL.ENCODER.get('SELECTIVE_DEPTH_THRESHOLD', 0.5)
            )
            if "itpnb" in name:
                num_channels = 512
            elif "itpnl" in name:
