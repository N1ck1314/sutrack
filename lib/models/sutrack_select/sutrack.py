"""
SUTrack Model with Selective Depth Integration
"""
import torch
import math
...
class SUTRACK_SELECT(SUTRACK):
    """ SUTrack with Selective Depth Integration """
    def __init__(self, text_encoder, encoder, decoder, task_decoder,
                 num_frames=1, num_template=1,
                 decoder_type="CENTER", task_feature_type="average",
                 use_selective_depth=False):
        super().__init__(text_encoder, encoder, decoder, task_decoder,
                        num_frames, num_template, decoder_type, task_feature_type)
        self.use_selective_depth = use_selective_depth
    
    def get_selection_loss(self):
        """获取选择性深度损失"""
        if self.use_selective_depth and hasattr(self.encoder, 'get_selection_loss'):
            return self.encoder.get_selection_loss()
        return 0.0


def build_sutrack_select(cfg):
