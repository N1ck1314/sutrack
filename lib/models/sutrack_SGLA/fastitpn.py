...
from lib.models.sutrack_SGLA.sgla_modules import SelectionModule, LayerAdaptiveWrapper
...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        # SGLA support
        self.use_sgla = use_sgla
        if self.use_sgla:
            from .sgla_modules import SelectionModule, LayerAdaptiveWrapper
            self.sgla_selector = SelectionModule(embed_dim, depth)
            self.layer_features = [] # 用于计算相似度损失
            print(f"SGLA enabled for Fast_iTPN with {depth} main blocks.")
    ...
    def build_blocks(self,
        ...
        if getattr(self, 'use_sgla', False):
            main_blocks = [LayerAdaptiveWrapper(blk) for blk in main_blocks]
            
        self.blocks.extend(main_blocks)
    ...
    def forward_features(self, template_list, search_list,template_anno_list, text_src, task_index):
        ...
        sgla_probs = None
        if self.use_sgla:
            sgla_probs = self.sgla_selector(xz)
            self.layer_features = [] # clear for this forward pass
            
        for i, blk in enumerate(self.blocks[-self.num_main_blocks:]):
            if self.use_sgla:
                prob = sgla_probs[:, i]
                xz = blk(xz, prob=prob, rel_pos_bias=rel_pos_bias)
                if self.training:
                    self.layer_features.append(xz)
            else:
