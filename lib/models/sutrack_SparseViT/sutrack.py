"""
SUTrack Model with SparseViT
基于SparseViT论文的稀疏自注意力增强版本

核心改进:
1. SABlock: 稀疏自注意力块，抑制语义干扰，强化非语义特征
2. 层级稀疏结构: 不同层采用不同稀疏率，捕获多尺度特征
3. 可学习特征融合: 自适应融合多尺度稀疏特征
"""
import torch
import math
...
class SUTRACK(nn.Module):
    """ This is the SUTrack with SparseViT for sparse attention enhancement """
    ...
    def forward_encoder(self, template_list, search_list, template_anno_list, text_src, task_index):
        # Forward the encoder (with SparseViT enhancement)
