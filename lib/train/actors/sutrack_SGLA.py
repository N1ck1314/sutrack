...
class SUTrack_SGLA_Actor(BaseActor):
    """ Actor for training the sutrack with SGLA support"""
    ...
    def forward_pass(self, data):
        ...
        # Get SGLA loss from encoder
        encoder = self.net.module.encoder if hasattr(self.net, 'module') else self.net.encoder
        if hasattr(encoder, 'get_sgla_loss'):
            self.sgla_loss = encoder.get_sgla_loss()
        else:
            self.sgla_loss = 0.0
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        # Add SGLA loss
        if hasattr(self, 'sgla_loss') and self.sgla_loss != 0.0:
            sgla_weight = self.cfg.MODEL.ENCODER.get('SGLA_LOSS_WEIGHT', 0.1)
            loss += self.sgla_loss * sgla_weight
        ...
        if return_status:
            ...
            if hasattr(self, 'sgla_loss') and self.sgla_loss != 0.0:
                status["Loss/sgla"] = self.sgla_loss.item() if torch.is_tensor(self.sgla_loss) else self.sgla_loss
